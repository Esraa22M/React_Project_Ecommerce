...
class Cart extends React.Component {
  ...
  render() {
    let { itemsAddedToCart, selectedCurrencyIndex,Tax , total ,quantity} = this.props;
    return (
      <Container>
                      ...
                      return [
                        ...
                        <div className="flex btn-container" key={index}>
                          ...
                          {item.type !== "swatch" ? (
                            ...
                            return (
                            <div  className={`btn-swatch ${
                              this.props.keepButtonActive(getId, item.id,product.selectedattributes)
                                ? "activeSwatchButton"
                                : ""
                            }   cursor-pointer btn-margin p-2`} key={getId+item.id}>  <ButtonDefault
        ...
        <div className="info">
          <p className="tax">Tax 21%:{Tax}</p>
          <p className="quantity">Quantity:{quantity}</p>
          <p className="total">Total:{total}{itemsAddedToCart[0].prices[selectedCurrencyIndex].currency.symbol}
</p>
          </div><hr/>
      </Container>
    );
