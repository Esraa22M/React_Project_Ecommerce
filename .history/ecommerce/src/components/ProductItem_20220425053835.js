   ...
   render() {
    let { gallery, name, prices, inStock, id } = this.props.product;
      ...
      <Product>
        ...
        {!inStock ? (
          <div className="div-2">        <LinkContainer to={`/details/${id}${this.props.categoryId}`}>
          OUT OF STOCK</LinkContainer></div>
        ) : (
          <Link
          to="/cart"
            className="card-icon"
            ...
            onClick={() => {
               let obj= this.getDefaultAttributesSelection(this.props.product)
  this.props.getItemsAddedToCart(id , this.props.categoryId , obj)
