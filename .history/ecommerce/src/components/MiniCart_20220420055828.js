    ...
    return (
      <MiniCartContainer clicked={this.props.isCartClicked ? "flex" : "none"}>
        ...
        {itemsAddedToCart.map((product, index) => {
          return (
            <article key={this.props.generateKey(index)} className="mini-cart-product-margin flex-container flex-space-between">
              <Column>
                ...
                <div className="attributes-container">
                  {product.attributes.map((item, index) => {
                    let getId = item.id;
                    return [
                      <h2 key={this.props.generateKey(index)} className="total heading">
                        {item.name + ":"}
                      </h2>,
              ...
              <div className="flex-container">
                <div className="counter-container">
                  <div className="counter-box flex-container" onClick={()=>this.props.onIncrement(product)}><img src={plusSign} alt="counter-sign-part-2" />
                        <img
                          src={minsSign}
                  ...
                  <div className="counter-box flex-container"onClick={()=>this.props.onDecrement(product)}>                <img
                          src={minsSign}
                          alt="counter-sign-part1"
                          className="sign"
                        /></div>
        ...
        <div className="flex-container flex-space-between margin-20">
          <Link to="/cart" className="view-cart-button" onClick={this.props.onCartClick}>View bag</Link>
          <Link to="/checkout" className="view-cart-button bg-primary" onClick={this.props.onCartClick}>CHECK OUT</Link>
