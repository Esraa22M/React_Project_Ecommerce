...
handlePrevImage = ()=>{
    ...
    render() {
        ...
        return <CartSliderContainer className='flex flex-center items-center'>
            ...
            {images.length>2?<div className='next flex items-center flex-center'onClick={this.handleNextImage}><img src={NextArrow} alt="next-arrow"/> </div>:<></>}
            {images.length>2?<div className='prev flex items-center flex-center' onClick={this.handlePrevImage}> <img src={PrevArrow}alt="prev-arrow" onClick={this.handlePrevImage}/></div>:<></>}
