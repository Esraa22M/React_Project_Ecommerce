...
class Cart extends React.Component {
  ...
  render() {
    ...
    return (
      <Container>
        <CartTitle className="flex flex-start items-center font-700">CART</CartTitle>
        <hr />
              ...
              <ProductContainer key={this.props.generateKey(index)} className=" flex flex-space-between">
                  ...
                  <div className="attributes-container">
                    {product.attributes.map((item, index) => {
                      ...
                      return [
                       ...
                       <div className="flex btn-container" key={index}> {item.type !== "swatch"
                          ? item.items.map((item, index) => {
                              return (
                                <ButtonDefault
                                  key={index+getId+item.id+(new Date().getMilliseconds())}
                                  className={` cursor-pointer btn-outline border-1 flex items-center flex-center attribute bg-light${
                          ...
                          : <div className="flex" key={index+(new Date().getMilliseconds())}>{ item.items.map((item, index) => {
                              return (
                                <div className={
                                  `p-2 ${ this.props.keepButtonActive(
                                     getId,
                                     item.id,
                                     product.selectedattributes
                                   )
                ...
                <Column direction="row" className="flex flex-column">
                  ...
                  <CartSlider images={product.gallery} />
                    ...
                    >delete</button>
                    <Link to={`/details/${product.id}${product.categoryId}`} className="btn btn-primary cursor-pointer text-upper-case text-primary">
                       Details
                    </Link>
                  </div>
                </Column>
