...
class App extends React.Component {
  ...
  handlePlusCounter = async(p) => {
    ...
    this.setState({ itemsAddedToCart: await cartProducts });
  };
  handleMinusCounter = async (p) => {
    let product = this.getingCartPorduct(p);
    if (!product.counter <= 0) {
      ...
      let index = this.getingCartPorductindex(p);
      let cartProducts = await [...this.state.itemsAddedToCart];
      cartProducts[index] = product;
